...
- `npm run smoke:api`: executa o smoke automatizado completo da API para `Condutor` e `Credenciada`.
...
O smoke valida os fluxos principais das APIs de `Condutor` e `Credenciada`:
...
Ele tambem aceita execucao manual por `workflow_dispatch`, com selecao de `all`, `condutor` ou `credenciada`.
